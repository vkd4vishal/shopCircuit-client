...
import { TextField, FormControlLabel, Checkbox, Button, Box, FormControl, InputLabel, MenuItem, Select } from "@mui/material";
...
const ItemMaintenanceFormView: React.FC<ItemMaintenanceProp> = ({ classes }) => {
  ...
  const [itemDetails,setItemDetails]= React.useState({itemName:'',brand:'',price:0,weight:0})
  ...
  const getItemDetails = () => {
    axios.get('/getItemDetails',{headers:{
      itemid: id
    }}).then((res) => {
  ...
  const getItemImage = () => {
    axios.get('/getItemImage',{headers:{
  ...
  return <div className={classes.root}>
    ...
    <form  onSubmit={handleSubmit}>
        ...
        <TextField
          ...
          onChange={(event)=>{
            setItemDetails({...itemDetails,price:Number(event.target.value)}) 
          }}
        />
         <TextField
          required
          ...
          onChange={(event)=>{
            setItemDetails({...itemDetails,weight:Number(event.target.value)}) 
          }}
        />
        <Box  >
              <FormControl >
